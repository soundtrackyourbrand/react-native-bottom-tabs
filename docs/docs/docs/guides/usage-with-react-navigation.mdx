...
Whether to hide the tab bar while the keyboard is shown. With Android's default `adjustResize` window mode the bar is otherwise lifted above the keyboard. Defaults to false.
...
<Tab.Screen
  ...
  options={{
    ...
    // or the name of a native image resource
    tabBarIcon: () => ({ uri: 'tab_person' }),
  }}
/>
```
...
:::tip
An icon referenced by name (`{ uri: 'tab_person' }`) is loaded from the app's native resources: a drawable on Android and an asset catalog image on iOS. These load synchronously, so the icons are part of the tab bar's first frame instead of appearing once an asynchronous load finishes. Vector drawables and template images take the tab tint like any other icon.
:::
