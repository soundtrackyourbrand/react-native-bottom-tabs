...
- `unfocusedIcon`: Icon to show when tab is inactive (optional). Icons referenced by name (`{ uri: 'tab_person' }`) are loaded synchronously from the app's native resources: a drawable on Android and an asset catalog image on iOS.
